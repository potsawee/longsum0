...
from data.loader import load_arxiv
...
ARXIV_DATA_DIR = "data/arxiv/"
OUTDIR         = "data/arxiv/oracle-padrand/4110/"
...
def filtering_data(start_id, end_id):
    ...
    for i in ids:
        ...
        # from dump_original
        dumped_path = "data/arxiv/list_{}/{}_original_input.bin".format(DATA_SET, i)
...
def combine():
    load_path = "{}/arxiv_{}.pk.bin".format(ARXIV_DATA_DIR, DATA_SET)
    with open(load_path, 'rb') as f:
        articles = pickle.load(f, encoding="bytes")
    print("len(articles) = {}".format(len(articles)))

    for i in tqdm(range(len(articles))):
        out_path = "data/arxiv/oracle-padrand//decode{}/{}_filtered_transcription.txt".format(MAX_BART_LEN, DATA_SET, i)

        with open(out_path, 'rb') as f:
            x = pickle.load(f, encoding="bytes")
        articles[i].article_text = x

    save_filtered_data_path = "data/arxiv/oracle-padrand/arxiv_set{}.bin".format(DATA_SET)
...
def dump_original(start_id, end_id):
    ...
    for i in ids:
        # check if the file exist or not
        # DECODER_DIR = temp folder
        out_path = "data/arxiv/list_{}/{}_original_input.bin".format(DATA_SET, i)
        exist = os.path.isfile(out_path)
        if exist:
            print("id {}: already exists".format(i))
            continue
...
if __name__ == "__main__":
    ...
    if(len(sys.argv) == 2):
        ...
        end_id   = start_id + 500
        if end_id > 203037: end_idx = 203037 # train=203037, val=6436, test=6440
